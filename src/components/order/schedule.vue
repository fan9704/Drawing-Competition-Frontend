...
import EditForm from '@/components/order/edit.vue';

import { listScheduleOrders, createOrder, updateOrder, deleteOrder } from '@/api/order';
...
const findAllOrders = async () => {
  const res = await listScheduleOrders();
...
const al = () => {
  swal.fire("刷新成功", "已成功刷新訂單排程頁面", "success")
  findAllOrders();
...
<template>
  ...
  <h1 id="panel-title">訂單排程</h1>
  <v-row>
    <v-btn @click="al">刷新清單</v-btn>
  </v-row>

  <v-row>
    <v-col cols="1">優先順序</v-col>
