...
const findAllChallenge = async () => {
  const res = await listChallenge();
...
<template>
    <v-container>
        ...
        <v-row v-for="(challenge,index) in teamChallenges">
            <v-col cols="4" v-if="selectKey==='' || selectKey==challenge.status">{{getChallengeStatusIcon(challenge.status)}}-{{ challenge.id }}</v-col>
            <v-col cols="4" v-if="selectKey==='' || selectKey==challenge.status" class="ellipsis" @click="gotoChallenge(challenge.id)" style="text-decoration: underline;">{{ challenge.description }}</v-col>
